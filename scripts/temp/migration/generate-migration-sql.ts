...
// This should be populated from event-mapping.ts if it exists
// For now, we'll detect event columns dynamically
...
async function processProfiles(rows: CSVRow[], eventColumns: string[]): Promise<{
  ...
  for (let i = 0; i < rows.length; i++) {
    ...
    const matchResult = await findExistingProfile(row);
    ...
    const processedProfile: ProcessedProfile = {
      csvRow: i + 1,
      csvData: row,
      dbMatch: matchResult.type === 'found' ? matchResult.profile : undefined,
      isNew: matchResult.type === 'not_found',
      mappedData: {
  ...
  const report: MigrationReport = {
    summary: {
      ...
      existingProfiles: profiles.filter(p => !p.isNew).length,
      newProfiles: profiles.filter(p => p.isNew).length,
...
function generateNewProfilesSQL(profiles: ProcessedProfile[]): string {
  ...
  if (newProfiles.length === 0) {
    return '-- No new profiles to insert\n';
  }
  
  let sql = 'BEGIN;\n\n';
  sql += '-- Insert new orphan profiles (without user_id)\n';
  sql += `-- Total: ${newProfiles.length} profiles\n\n`;
...
function generateEventParticipantsSQL(profiles: ProcessedProfile[], eventMapping: Record<string, string>): string {
  // For new profiles, we'll need to reference them by a subquery
  // For existing profiles, we have their IDs
  
  let sql = 'BEGIN;\n\n';
  sql += '-- Insert event participations\n';
  sql += '-- Note: For new profiles, we reference them by email/phone\n\n';
  ...
  for (const profile of profiles) {
    for (const [eventColumn, participated] of Object.entries(profile.events)) {
      ...
      if (profile.isNew) {
        // For new profiles, we need to find them by email/phone
        sql += `-- Participation for new profile (row ${profile.csvRow}) in event "${eventColumn}"\n`;
      ...
      } else if (profile.dbMatch) {
        // For existing profiles, we have the ID
        sql += `-- Participation for existing profile ${profile.dbMatch.id} in event "${eventColumn}"\n`;
        sql += 'INSERT INTO event_participants (\n';
        sql += '  id, event_id, profile_id, status, attendance_confirmed,\n';
        sql += '  created_at, updated_at\n';
        sql += ') VALUES (\n';
...
async function main() {
  ...
  console.info('🚀 Positiv Profile Migration SQL Generator');
  console.info('==========================================');
  ...
  try {
    ...
    console.info(`Existing Profiles: ${report.summary.existingProfiles}`);
    console.info(`New Profiles to Create: ${report.summary.newProfiles}`);
    ...
    if (generateSQL) {
      ...
      console.info('\nNext steps:');
      console.info('1. Review the generated SQL files');
      console.info('2. Run the migration script: bash scripts/temp/migration/execute-migration.sh');
    } else {
