...
--
-- Until POS-523 this file derived each row from event_participants.has_paid
-- and .payment, mirroring 20260827143204_backfill_payments.sql. POS-524 dropped
-- those columns, so the rule stands on its own: a participation that finalised
-- was paid for, unless the person withdrew or was sent to the rodizio. That
-- keeps the live events seeded with money too -- their participants have not
-- attended anything yet, so a rule resting on attendance alone would leave the
-- admin grid empty exactly where an admin looks. Everything before the ledger
-- existed was a PIX transfer arranged by hand, which is what this produces. A
-- staff or social spot pays nothing and is settled all the same, so it gets a
-- payment of zero -- see
