...
export class EventApplicationPage extends BasePage {
  ...
  constructor(page: Page) {
    ...
    // Rules page
    this.rulesTitle = page.getByRole('heading', { name: 'Regras e filosofias', exact: true })
    this.rulesTestTitle = page.getByRole('heading', { name: '✅ Hora do teste! ✅', exact: true })
    this.continueButton = page.getByRole('button', { name: 'Continuar' })
    this.requiredError = page.getByText('Há erros nas suas respostas', { exact: true })
    this.questions = page.locator('div[data-testid="question"]')

    // BDSM consent page
    this.bdsmTitle = page.getByRole('heading', { name: 'Consentimento BDSM', exact: true })
    this.bdsmContinueButton = page.getByRole('button', { name: 'Continuar' })
    this.bdsmCheckbox = page.getByRole('checkbox')

    // User data page
    this.userDataTitle = page.getByRole('heading', { name: 'Quase lá!', exact: true })
    this.notesTextbox = page.getByRole('textbox', { name: /nota ou/ })
    this.bondRadioButtons = page.locator('input[type="radio"]')
    this.confirmButton = page.getByRole('button', { name: '🎉 Confirmar Inscrição!' })
  }
  ...
  async fillRulesForm(): Promise<void> {
    ...
    const uncheckedRadios = await this.page.locator('div[data-testid="question"] input[type="radio"]:not(:checked)').all()
    ...
    for (const radio of uncheckedRadios) {
      ...
      if (name && !processedGroups.has(name)) {
        ...
        const lastInGroup = this.page.locator(`input[type="radio"][name="${name}"]`).last()
  ...
  async fillUserDataForm(notes?: string, bondType?: string): Promise<void> {
  ...
  async completeFullApplication(options?: {
    ...
    // Fill user data form
    await this.fillUserDataForm(options?.notes || 'Test application notes', options?.bondType)
