...
export async function createTestEventWithParticipants(eventId: string, count: number = 3): Promise<TestParticipant[]> {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const profileData = {
      ...
      flag: (i === 0 ? 'yellow' : i === 1 ? 'red' : 'none') as 'none' | 'yellow' | 'red', // Vary flag colors
      flag_notes: i === 0 ? 'Test flag note' : null,
      approved_to_attend: (i === 0 ? 'approved' : 'pending') as 'approved' | 'pending'
    }
    ...
    const participantData = {
      ...
      application_status: (i === 0 ? 'finalised' : i === 1 ? 'pending' : 'talking') as 'pending' | 'talking' | 'sent_payment_data' | 'sent_rules' | 'think_better' | 'finalised',
      attendance_status: (i === 0 ? 'attended' : 'pending') as 'pending' | 'attended' | 'not-attended' | 'skipped' | 'will-not-go',
      has_paid: i === 0,
      payment: i === 0 ? 100 : 0,
      spot_type: (i === 0 ? 'regular' : i === 1 ? 'social' : 'staff') as 'regular' | 'social' | 'staff'
    }
...
export async function cleanupTestParticipants(participants: TestParticipant[]): Promise<void> {
  const supabase = createSupabaseAdminClient()
