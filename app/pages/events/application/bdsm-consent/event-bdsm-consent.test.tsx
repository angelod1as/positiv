...
describe("EventBdsmConsentPage", () => {
  ...
  it("renders all content sections", () => {
    ...
    expect(screen.getByText(/Antes de começar, gostaríamos de deixar claro/)).toBeInTheDocument()
    expect(screen.getByRole("heading", { name: "BDSM essentials" })).toBeInTheDocument()
  ...
  it("displays the Alert component with correct variant", () => {
    ...
    expect(alert).toHaveTextContent(/terminante proibido o uso de qualquer substância/)
  })
  ...
  it("renders position descriptions correctly", () => {
    ...
    expect(screen.getByText(/pessoa que está como receptora/)).toBeInTheDocument()
    expect(screen.getByText(/pessoa que gosta de ser tanto/)).toBeInTheDocument()
  })
  ...
  it("renders the consent checkbox unchecked by default", () => {
    ...
    const checkbox = screen.getByRole("checkbox", { name: "Estou ciente e quero continuar" })
  ...
  it("shows validation error when trying to submit without consent", async () => {
    ...
    expect(screen.getByText("Você deve aceitar para continuar")).toBeInTheDocument()
  })
  ...
  it("allows form submission when consent is checked", async () => {
    ...
    expect(screen.queryByText("Você deve aceitar para continuar")).not.toBeInTheDocument()
  })

  it("renders all important warnings and aftercare information", () => {
    render(<EventBdsmConsentPage />)
    
    expect(screen.getByText("Vamos começar do começo: Não existe BDSM sem consentimento!")).toBeInTheDocument()
